...
def run_fastp(Configuration):
    ...
    if len(lanes) > 1:
        logging.info(f"Merging multiple lanes")
        output_dir = os.path.join(Configuration.Trimmed_dir,Configuration.file_to_process)
        merged_ouput_file_R1 = os.path.join(output_dir,Configuration.file_to_process) + "_trimmed_R1.fastq.gz"
        merged_ouput_file_R2 = os.path.join(output_dir,Configuration.file_to_process) + "_trimmed_R2.fastq.gz"
        
        trimmed_individual_R1 = []
        trimmed_individual_R2 = []
        for lane in lanes:
            trimmed_individual_R1.append(os.path.join(output_dir,lane)+"R1.fastq.gz")
            trimmed_individual_R2.append(os.path.join(output_dir,lane)+"R2.fastq.gz")

        with open(merged_ouput_file_R1, "w") as combined_output:
            subprocess.run(f"cat {' '.join(trimmed_individual_R1)}",shell=True,stdout=combined_output, cwd = output_dir)
            subprocess.run(f"rm {' '.join(trimmed_individual_R1)}",shell=True, cwd = output_dir)
        with open(merged_ouput_file_R2, "w") as combined_output:
            subprocess.run(f"cat {' '.join(trimmed_individual_R2)}",shell=True,stdout=combined_output, cwd = output_dir)
            subprocess.run(f"rm {' '.join(trimmed_individual_R2)}",shell=True, cwd = output_dir)

        logging.info("Finished merging lanes")
